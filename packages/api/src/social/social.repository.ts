...
export class SocialRepository {
  ...
  async getLikeCount(trackId: string): Promise<number> {
    const { data, error } = await this.client.rpc("get_like_count", {
      p_track_id: trackId,
    });
    if (error) throw error;
  ...
  async getUserFavorites(userId: string, limit = DEFAULT_LIST_LIMIT, offset = 0): Promise<Favorite[]> {
  ...
  async toggleFollow(entityType: FollowEntityType, entityId: string): Promise<boolean> {
    const { data, error } = await this.client.rpc("toggle_follow", {
      p_entity_type: entityType,
      p_entity_id: entityId,
    });
    if (error) throw error;
  ...
  async isFollowing(entityType: FollowEntityType, entityId: string): Promise<boolean> {
    const { data, error } = await this.client.rpc("is_following", {
      p_entity_type: entityType,
      p_entity_id: entityId,
    });
    if (error) throw error;
  ...
  async getFollowCount(entityType: FollowEntityType, entityId: string): Promise<number> {
    const { data, error } = await this.client.rpc("get_follow_count", {
      p_entity_type: entityType,
      p_entity_id: entityId,
    });
    if (error) throw error;
  ...
  async getUserFollows(userId: string, limit = DEFAULT_LIST_LIMIT, offset = 0): Promise<Follow[]> {
    const { data, error } = await this.client
      .from("follows")
      .select("*")
  ...
  async getEngagementStats(entityType: string, entityId: string): Promise<EngagementStats> {
    const { data, error } = await this.client.rpc("get_engagement_stats", {
      p_entity_type: entityType,
      p_entity_id: entityId,
    });
    if (error) throw error;
    const result = data as unknown as EngagementStats;
    return {
  ...
  async getCreatorEngagementStats(creatorId: string): Promise<CreatorEngagementStats> {
    const { data, error } = await this.client.rpc("get_creator_engagement_stats", {
      p_creator_id: creatorId,
    });
    if (error) throw error;
    const result = data as unknown as CreatorEngagementStats;
    return {
