-- Sprint 4.1 — Anti-fraude streaming : abandon des sessions orphelines
-- Modifie start_stream_session pour fermer automatiquement les sessions mortes
-- (heartbeat absent depuis > 5 min) avant d'en créer une nouvelle.
-- Cela évite les sessions zombies qui polluent les analytics et royalties.
...
BEGIN
  ...
  -- Anti-fraude : fermer les sessions orphelines de cet utilisateur.
  -- Une session est orpheline si elle n'a pas reçu de heartbeat depuis > 5 minutes.
  -- Les sessions légitimes envoient un heartbeat toutes les 10 secondes.
  UPDATE public.stream_sessions
  SET completed_at = now(),
      is_valid_listen = false,
      fraud_flags = array_append(fraud_flags, 'orphaned_session')
