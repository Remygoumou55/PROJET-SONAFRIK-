-- Alignement garde DB avec le service layer : integrity_status = 'valid' uniquement.
-- Le service assertAlbumReadyForSubmit() n'accepte déjà que 'valid'.
-- Ce patch aligne le RPC pour éviter qu'un bypass direct de l'API puisse soumettre
-- un album dont l'audio est encore en état 'pending'.
BEGIN;
...
BEGIN
  ...
  -- Garde strict : uniquement 'valid' — aligné avec assertAlbumReadyForSubmit()
