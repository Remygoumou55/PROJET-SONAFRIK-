...
CREATE POLICY genres_select_active ON public.genres
...
CREATE POLICY albums_insert_edit ON public.albums
  FOR INSERT TO authenticated
  WITH CHECK (public.can_edit_creator(creator_id));

CREATE POLICY albums_update_edit ON public.albums
  FOR UPDATE TO authenticated
  USING (public.can_edit_creator(creator_id))
  WITH CHECK (public.can_edit_creator(creator_id));

-- album_genres
CREATE POLICY album_genres_select ON public.album_genres
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY album_genres_manage ON public.album_genres
  FOR ALL TO authenticated
  USING (
    EXISTS (
...
CREATE POLICY tracks_insert_edit ON public.tracks
  FOR INSERT TO authenticated
  WITH CHECK (public.can_edit_creator(creator_id));

CREATE POLICY tracks_update_edit ON public.tracks
  FOR UPDATE TO authenticated
  USING (public.can_edit_creator(creator_id))
  WITH CHECK (public.can_edit_creator(creator_id));

-- track_genres
CREATE POLICY track_genres_select ON public.track_genres
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY track_genres_manage ON public.track_genres
  FOR ALL TO authenticated
  USING (
    EXISTS (
...
-- track_files
CREATE POLICY track_files_select_member ON public.track_files
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.tracks t
...
CREATE POLICY track_files_insert_edit ON public.track_files
  FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.tracks t
...
CREATE POLICY track_files_update_edit ON public.track_files
  FOR UPDATE TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.tracks t
...
CREATE POLICY catalog_audio_select ON storage.objects
  FOR SELECT TO authenticated
  USING (
...
CREATE POLICY catalog_audio_insert ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (
    bucket_id = 'catalog-audio'
...
CREATE POLICY catalog_audio_update ON storage.objects
  FOR UPDATE TO authenticated
  USING (
    bucket_id = 'catalog-audio'
    AND public.can_edit_creator(((storage.foldername(name))[1])::uuid)
...
CREATE POLICY catalog_visuals_select ON storage.objects
  FOR SELECT TO authenticated
  USING (
...
CREATE POLICY catalog_visuals_insert ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (
    bucket_id = 'catalog-visuals'
    AND public.can_edit_creator(((storage.foldername(name))[1])::uuid)
...
CREATE POLICY catalog_visuals_update ON storage.objects
  FOR UPDATE TO authenticated
  USING (
    bucket_id = 'catalog-visuals'
    AND public.can_edit_creator(((storage.foldername(name))[1])::uuid)
