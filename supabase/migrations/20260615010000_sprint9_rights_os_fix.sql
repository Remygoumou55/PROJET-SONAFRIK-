...
-- ─── 1. Supprimer les anciennes politiques RLS (si elles existent) ────────────
...
DECLARE
  ...
  tbl TEXT;
BEGIN
  FOR tbl IN SELECT unnest(ARRAY['works','contributors','ownerships','ownership_versions','contracts','rights_claims'])
  LOOP
    FOR pol IN
...
-- ─── 2. Supprimer les anciennes tables Rights OS (pour repartir propre) ───────
-- ATTENTION : ceci supprime les données existantes dans ces tables.
-- Accepté car les tables sont vides en pre-prod.

DROP TABLE IF EXISTS rights_claims       CASCADE;
DROP TABLE IF EXISTS ownership_versions  CASCADE;
DROP TABLE IF EXISTS ownerships          CASCADE;
DROP TABLE IF EXISTS contributors        CASCADE;
DROP TABLE IF EXISTS contracts           CASCADE;
DROP TABLE IF EXISTS works               CASCADE;

-- ─── 3. Supprimer les anciens types enum (si existants) ───────────────────────
...
DROP TYPE IF EXISTS rights_claim_type  CASCADE;
DROP TYPE IF EXISTS rights_claim_status CASCADE;

-- ─── 4. Supprimer l'ancienne fonction trigger (si elle existe) ────────────────

DROP FUNCTION IF EXISTS check_ownership_total() CASCADE;

-- ─── 5. Recréer les types énumérés ───────────────────────────────────────────
...
-- La fonction set_updated_at() est déjà créée dans une migration précédente.
-- On recrée uniquement les triggers sur les nouvelles tables.
