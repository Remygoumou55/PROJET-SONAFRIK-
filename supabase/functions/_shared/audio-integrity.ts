...
export function validateAudioAsset(input: {
  ...
  if (fileSizeBytes > MAX_UPLOAD_BYTES) return { status: "invalid", message: "Fichier > 50 Mo.", container: "unknown", webCompatible: false };
  ...
  if (container === "wav" || container === "flac" || dbFormat === "wav" || dbFormat === "flac") {
    return { status: "needs_review", message: "WAV/FLAC — transcodage requis.", container, webCompatible: false };
  }
