    ...
    <div className="flex items-center gap-3 py-3 px-3" style={{ borderBottom: "1px solid #141414" }}>
      {hasThumb && (
        <div
          ...
          style={{ backgroundColor: "#1F1F1F", animationDelay: `${delayMs}ms` }}
        />
      )}
      <div className="flex-1 space-y-2">
        <div
          className="h-3 rounded-full animate-pulse"
          style={{ backgroundColor: "#1F1F1F", width: "60%", animationDelay: `${delayMs}ms` }}
        />
        <div
          className="h-2.5 rounded-full animate-pulse"
          style={{ backgroundColor: "#1A1A1A", width: "40%", animationDelay: `${delayMs + 40}ms` }}
        />
      </div>
      <div
        className="w-10 h-3 rounded-full animate-pulse flex-shrink-0"
        style={{ backgroundColor: "#1F1F1F", animationDelay: `${delayMs + 60}ms` }}
      />
    </div>
  );
}
