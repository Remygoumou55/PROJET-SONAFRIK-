...
function Badge({ label, color = "#555555" }: { label: string; color?: string }) {
  return (
    <span
    ...
    <div className="space-y-6">
      ...
      <div style={cardStyle}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold" style={{ color: "#FFFFFF" }}>{work.title}</h2>
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
              ...
              {work.genre && <Badge label={work.genre} color="#A0A0A0" />}
              <Badge label={work.language.toUpperCase()} color="#555555" />
            </div>
            {work.description && (
              <p className="text-sm mt-2" style={{ color: "#A0A0A0" }}>{work.description}</p>
            )}
          </div>
          <div className="text-right shrink-0">
            <p className="text-xs" style={{ color: "#555555" }}>Parts déclarées</p>
            <p
              className="text-2xl font-black tabular-nums"
              style={{ color: totalShare >= 100 ? "#00D26A" : "#FFC20E" }}
            >
              {totalShare.toFixed(0)}%
            </p>
            {totalShare < 100 && (
              <p className="text-xs mt-0.5" style={{ color: "#FFC20E" }}>
      ...
      <div className="flex gap-1 border-b" style={{ borderColor: "#2A2A2A" }}>
        {tabs.map(({ key, label }) => (
          <button
            ...
            style={{
              borderColor: activeTab === key ? "#00D26A" : "transparent",
              color: activeTab === key ? "#00D26A" : "#555555",
            }}
          >
            {label}
          </button>
              ...
              return (
                <div key={c.id} style={cardStyle}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {masterShare > 0 && (
                      <div className="text-right shrink-0">
                        <p className="text-lg font-bold tabular-nums" style={{ color: "#FFC20E" }}>
                  ...
                  {c.ownerships.filter((o) => o.ownership_type !== "master").map((o) => (
                    <div key={o.id} className="flex justify-between mt-2 pt-2" style={{ borderTop: "1px solid #2A2A2A" }}>
                      <span className="text-xs" style={{ color: "#A0A0A0" }}>
                        {OWNERSHIP_TYPE_LABELS[o.ownership_type as OwnershipType] ?? o.ownership_type}
                        {o.territory !== "worldwide" ? ` — ${o.territory}` : ""}
                      </span>
                      <span className="text-xs font-semibold" style={{ color: "#FFC20E" }}>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      <Badge
                        ...
                        color="#FFC20E"
                      />
                      {c.signed_at && <Badge label="Signé" color="#00D26A" />}
                    </div>
                    {c.start_date && (
                  ...
                  {c.revenue_share_percent != null && (
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-xs" style={{ color: "#555555" }}>revenue share</p>
                    </div>
                  )}
                </div>
                {c.terms && (
                  <p className="text-xs mt-2 pt-2 line-clamp-2" style={{ color: "#A0A0A0", borderTop: "1px solid #2A2A2A" }}>
                    {c.terms}
                  </p>
                )}
              </div>
      ...
      {activeTab === "claims" && (
        <div className="space-y-3">
          {work.claims.length === 0 ? (
            ...
            work.claims.map((cl) => {
              const statusColor =
                cl.status === "accepted" ? "#00D26A" :
                cl.status === "rejected" ? "#FF6666" :
                cl.status === "escalated" ? "#FFC20E" : "#A0A0A0";
              return (
